...
<div id="container">
	...
	<div id="formArea">
		<form name="scheduleForm" action="${pageContext.request.contextPath }/admin/program/schedule_register" method="post">
			...
			<table id="inputTable">
				...
				<tr>
					<th>일정</th>
					<td>
						<input class="schedule-input" type="text" id="datepicker" name="departureDate" placeholder="출발일을 선택하세요" autocomplete="off">
				...
				<tr>
					...
					<th>봉사자</th>
					<td>
						<input class="schedule-input" type="text" id="volunteer" name="volunteer" onblur="volunteerFn();">
						<button class="program-btn" id="searchBtn" type="button" onclick="searchVol();">검색</button>
					</td>
				</tr>
				...
				<tr>
					<td class="noticeArea" colspan="2">
						<textarea rows="30" cols="130" name="scheduleContent" id="scheduleContent"></textarea>
					</td>	
				</tr>
			</table>
			<div id="buttonArea">
				<button class="program-btn action-btn" type="button" id="regSubBtn" onclick="validate();">제출</button>
				<button class="program-btn-r action-btn" type="button" onclick="location.href='#${pageContext.request.contextPath}/admin/program/schedule_register';">취소</button>
			</div>
		</form>
	</div>
</div>
...
<div id="secretCont">
	<h1>Error</h1>
	<p>현재 시스템 점검중입니다.</p>
	<p>'검색'대신 봉사자 이름을 직접 입력해주세요</p>	
	<button type="button" onclick="closeThis();">확인</button>
</div>
...
<script type="text/javascript">
	...
	*/
	...
	function getNotice(){
		...
		if(!areaList.contains("contentSelf")){
			...
			$.ajax({
				...
				data: JSON.stringify({"programType": selectValue}),
				success: function(text) {
					document.getElementById("scheduleContent").value=text;
				},
				error: function(xhr){
	...
	function validate(){
		...
		if(!confirm("입력하신 정보로 스케쥴을 등록하시겠습니까?")){
			return;
		}
	...
	function searchVol() {
		...
		document.getElementById("secretCont").style.display="block";
	}
	
	function closeThis() {
		document.getElementById("container").style.display="block";
		document.getElementById("secretCont").style.display="none";
