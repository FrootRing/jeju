	...
	<div class="wrap">
		<div class="container">
			<table style="width: 1100px; margin-bottom: 20px;">
				...
				<%-- <c:forEach var="i" begin="0" end="${fn:length(festivalList)-1 }" step="1"> --%>					
				<c:forEach var="festivalList" items="${festivalList}">
				<tr>
					<%-- <c:set var="festival_no" value="${festivalList[i].no }"/> --%>
					<%-- <fmt:parseNumber var="no" integerOnly="true" type="number" value="${festival_no }" /> --%>
		...
		<div id="blockArea">
			<ul>
				<c:if test="${pager.pageNum != 1 }">
					<li><a href="${pageContext.request.contextPath }/addFestival/${pager.pageNum-1}">이전</a></li>
				</c:if>
				...
				<c:forEach var="i" begin="${pager.startPage }" end="${pager.endPage }" step="1">
					<c:choose>
						...
						<c:otherwise>
							<li><a href="${pageContext.request.contextPath }/addFestival/${i}">${i }</a></li>
						</c:otherwise>				
					</c:choose>
				</c:forEach>
			
				<c:if test="${pager.pageNum != pager.endPage }">
					<li><a href="${pageContext.request.contextPath }/addFestival/${pager.pageNum+1}">다음</a></li>
				</c:if>
			</ul>
		</div>
